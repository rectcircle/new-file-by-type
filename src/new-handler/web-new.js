...
function getExtResList(projectDir, srcPath) {
    ...
    return util.traverseDir(p, (subPath, isFile) => {
...
async function inputFileInfo({ //工作空间
    ...
    if (WEB_TYPES[0] == subType) { //HTML
        ...
        let extResList = await selectMany(
            getExtResList(util.pathResolve(srcDirPath, '../'), srcDirPath.substr(path.dirname(srcDirPath).length+1)),
            langPack.inputExtResList)
        if(extResList==undefined) return undefined
        //输入页面标题
        ...
        let targetPath = util.pathResolve(srcDirPath, `${fileName}.html`)
        return {
            targetPath,
            code
        }
